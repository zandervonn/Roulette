    ...
    if c is not None and len(c) > 1:
        ...
        if lower < t_a < upper:
            if not on:
                ...
                # print("angle check = ",lower, "to", upper, ", angle found = ", t_a)
...
def main():
    ...
    while i < len(ball_arr_log) - 1:
        # print("real val= ", ball_arr_log[i])
        # print("guess = ", best_fit(times[i]))
    ...
    while True:
        i = 0
        print("pop bottom", len(ball_arr_log), "i = ", i, " guess = ", best_fit(times[i]), " real = ", ball_arr_log[i],
              "avg = ", avg * range_t)
    ...
    while True:
        ...
        print("pop top", len(ball_arr_log), "i = ", i, " guess = ", best_fit(times[i]), " real = ", ball_arr_log[i],
              "avg = ", avg * range_t)
        if best_fit(times[i]) - (avg * range_t) > ball_arr_log[i] or best_fit(times[i]) + (avg * range_t) < ball_arr_log[i]:
            ball_arr_log.pop(i)
            times.pop(i)
    ...
    print("rps start = ", (1000 / (e ** best_fit(0))))

    print("rps at fall = ", (1000 / (e ** best_fit(time_total))))
